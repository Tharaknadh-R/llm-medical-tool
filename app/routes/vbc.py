...
from app.prompts.prompts import VBC_Notes
from app.data.requirements import REQUIREMENTS

router = APIRouter()

@router.post("/vbc-notes")
async def generate_vbc_notes(request: ClinicalRequest):
    response = await run_in_threadpool(
        generate_clinical_response,
        appointment_date=request.appointment_date,
        appointment_id=request.appointment_id,
        question_type=VBC_Notes
    )

    REQUIREMENTS["vbc_notes"].append({
        "appointment_date": request.appointment_date,
        "appointment_id": request.appointment_id,
        "response": response
    })
